...
def _extract_words_from_book(book_pages, book_list, lock):
    ...
    with counter.get_lock():
        _id = counter.value
        counter.value += 1
    book = Book(_id, book_pages[0], book_pages[1])
...
def _get_all_books():
    ...
    l = m.Lock()
    book_list = m.list()
    partial_extract_words_from_book = partial(_extract_words_from_book, book_list=book_list, lock=l)
